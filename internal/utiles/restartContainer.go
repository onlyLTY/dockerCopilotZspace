...
func RestartContainer(ctx *svc.ServiceContext, id string) error {
	...
	url := baseURL + "/docker/containers/" + id + "/restart"
	...
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusNoContent && resp.StatusCode != http.StatusNotModified {
		...
		logx.Errorf("RestartContainer error: %v", errorResponse.Message)
		return fmt.Errorf("RestartContainer error: %v", errorResponse.Message)
	}
	return nil
}
