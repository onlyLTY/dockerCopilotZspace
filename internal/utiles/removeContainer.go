...
func RemoveContainer(ctx *svc.ServiceContext, id string) error {
	...
	url := baseURL + "/docker/containers/" + id + "?v=1"
	...
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusNoContent && resp.StatusCode != http.StatusNotModified {
		...
		logx.Errorf("RemoveContainer error: %v", errorResponse.Message)
		return fmt.Errorf("RemoveContainer error: %v", errorResponse.Message)
	}
	return nil
}
